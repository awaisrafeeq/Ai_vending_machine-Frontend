...
export default function Page() {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        const response = await fetch(`https://ai-vending-machine-4fwl.onrender.com/product?id=${id}`);
        if (!response.ok) {
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          const response = await fetch('https://ai-vending-machine-4fwl.onrender.com/analyze-audio', {
            method: 'POST',
            headers: {
      ...
      if (ctx) {
        ...
        canvas.toBlob(async (blob) => {
          if (blob) {
            ...
            try {
              const response = await fetch('https://ai-vending-machine-4fwl.onrender.com/analyze-image', {
                method: 'POST',
                headers: {
                  'Accept': 'application/json',
                },
